# src/eegcfct/ssl/contrastive_over_channel.py
import torch
...
class TinyChLSTMEncoder(nn.Module):
    ...
    Channel-wise temporal encoder:
      ...
      Output: (B, C, D)
    """
    def __init__(self, in_ch: int, emb_dim: int = 32, hidden: int = 16):
        super().__init__()
        self.hidden = hidden
        self.conv = nn.Sequential(
            nn.Conv1d(in_ch, in_ch, kernel_size=5, padding=2, groups=in_ch, bias=False),
        ...
        # LSTM processes per-channel sequence (feature size 1)
        self.lstm = nn.LSTM(input_size=1, hidden_size=hidden,
                            bidirectional=True, batch_first=False)
        self.lin = nn.Linear(2 * hidden, emb_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, C, T)
        B, C, T = x.shape
        h = self.conv(x)                       # (B, C, T)
        h = h.reshape(B * C, T, 1).transpose(0, 1)  # (T, B*C, 1)
...
# -------- SSL helpers --------
def _random_crop_pair(x: torch.Tensor, crop_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
    T = x.shape[-1]
    crop_len = min(crop_len, T)
    if crop_len == T:
        return x, x
    max_s = T - crop_len + 1
    s1 = torch.randint(0, max_s, (1,), device=x.device).item()
    s2 = torch.randint(0, max_s, (1,), device=x.device).item()
    return x[..., s1:s1+crop_len], x[..., s2:s2+crop_len]

def _nt_xent(z1: torch.Tensor, z2: torch.Tensor, tau: float = 0.2) -> torch.Tensor:
    B, C, D = z1.shape
    z1 = F.normalize(z1.reshape(B * C, D), dim=-1)
    z2 = F.normalize(z2.reshape(B * C, D), dim=-1)
    reps = torch.cat([z1, z2], dim=0)       # (2N, D) with N=B*C
    ...
    *,
    ...
    for ep in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            v1, v2 = _random_crop_pair(X, crop_len)
            z1, z2 = enc(v1), enc(v2)
            loss = _nt_xent(z1, z2, tau=0.2)
...
def _pca_basis_from_raw(windows_ds, chan_idx: np.ndarray, *, pcs_per_cluster: int,
    ...
    while got < samples:
        ...
        X = batch[0][:, chan_idx, :].cpu().numpy()            # (B, n_c, T)
        X_blocks.append(X.transpose(1, 0, 2).reshape(len(chan_idx), -1))  # (n_c, B*T)
        got += 1
    Xcat = np.concatenate(X_blocks, axis=1)                   # (n_c, bigT)
    Xcat = (Xcat - Xcat.mean(axis=1, keepdims=True)) / (Xcat.std(axis=1, keepdims=True) + 1e-8)
    p = min(pcs_per_cluster, Xcat.shape[0])
    U = PCA(n_components=p, svd_solver="full").fit(Xcat.T).components_.T  # (n_c, p)
    return U
...
def build_channel_projection_from_ssl(
    ...
    *,
    ...
    # 1) Get C and D
    probe = DataLoader(windows_ds, batch_size=1, shuffle=True)
    X0 = next(iter(probe))[0].to(device).float()     # (1, C, T)
    C = X0.shape[1]
    D = encoder(torch.zeros(1, C, min(150, X0.shape[-1]), device=device)).shape[-1]

    # 2) Embed channels over a few random crops
    dl = DataLoader(windows_ds, batch_size=16, shuffle=True, drop_last=True)
    it = iter(dl)
    ...
    for _ in range(64):
        try:
            ...
            it = iter(dl); batch = next(it)
        X = batch[0].to(device).float()
        v1, _ = _random_crop_pair(X, min(150, X.shape[-1]))
        z = encoder(v1)                 # (B, C, D)
        acc += z.mean(dim=0)            # (C, D)
        n += 1
    emb = (acc / max(n, 1)).detach().cpu().numpy()   # (C, D)

    # 3) KMeans on channel embeddings
    km = KMeans(n_clusters=n_clusters, n_init=10, random_state=0)
    labels = km.fit_predict(emb)

    # 4) PCA per cluster → assemble W (sum_pcs x C)
    out_rows = 0; bases = []
    for g in range(n_clusters):
        idx = np.where(labels == g)[0]
        if len(idx) == 0:
            bases.append((idx, np.zeros((0, 0), dtype=np.float32))); continue
        U = _pca_basis_from_raw(windows_ds, idx, pcs_per_cluster=pcs_per_cluster)  # (n_c, p)
        bases.append((idx, U)); out_rows += U.shape[1]
